	...
	$.extend($.fn, {
		jpagebar: function (setting) {
			...
			pb.resetPagebar = function () {
				...
				//				_this.totalpage = 12;
				render_to.append('<div style="margin: 0 auto;float: right;margin-right: 5%"></div>');
				render_to = render_to.find('div');

				$('<div class="page-info">共&nbsp;' + _this.totalpage + '&nbsp;页&nbsp;,&nbsp;' + _this.totalcount + '&nbsp;条记录</div>').appendTo(render_to);
				var pagebar = $('<ul></ul>');
...
var _newPaging = function (result, page, fun) {
	var totalPage = result.totalPages || result.totalPage, totalCount = result.totalElements || result.totalCount, currentPage = result.currentPage
	if (page != 0) {/*totalPage >= 2 && */
		$(function () {
			$.fn.jpagebar({
				renderTo: $("#mt_page"),
				totalpage: totalPage,
