...
angular.module('ywsApp')
    .controller('CoursePlanMultipleController', ['$scope', '$location', 'CoursePlanService', '$modal', '$rootScope', 'SweetAlert', 'UserPrizeService',
        'CustomerStudentGroupService', 'LeadsStudentService', 'CustomerStudentCourseService', 'CommonService', 'OrderService',
        '$routeParams', 'AuthenticationService', 'ClassStudentAttendenceService', '$mtModal', 'CustomerStudentService', 'ClassManagementService', 'localStorageService',
        function ($scope, $location, CoursePlanService, $modal, $rootScope, SweetAlert, UserPrizeService, CustomerStudentGroupService, LeadsStudentService, CustomerStudentCourseService,
            CommonService, OrderService, $routeParams, AuthenticationService, ClassStudentAttendenceService, $mtModal, CustomerStudentService, ClassManagementService, localStorageService) {
             ...
             */
            //获取schoolid的
            $scope.schoolId = AuthenticationService.currentUser().school_id
            $scope.subjectIds = [];
            $scope.gradeIds = [];
            $scope.courseTypeIds = [];
            ...
            $scope.teachernow = function teachernow() {
                ...
                CoursePlanService.teachernow(start, number, $scope.teachername).then(function (result) {
                    ...
                    console.log($scope.teacherdisplayed)
                });
            };
            ...
            $scope.getClassPorintList = function (omsCoursePlan) {
                CoursePlanService.classPorintListmore(angular.copy(omsCoursePlan)).then(function (data) {
            ...
            $scope.ListencallServerrecord = function ListencallServerrecord(tableState) {
                ...
                ta.search.predicateObject.schoolId = $scope.schoolId;

                CoursePlanService.Listenrecordmultiplelist(start, number, ta).then(function (result) {
                    $scope.displayedrecord = result.data;
                    angular.forEach($scope.displayedrecord, function (p, index) {
                        p.studentNamesStr = p.studentNames.join(' ')
                    });
                    $scope.myCoursePlanRecordTableState = tableState;
                    $scope.num = result.numberOfPages;
                    tableState.pagination.numberOfPages = result.numberOfPages;//set the number of pages so the pagination can update
            ...
            function addUnsatisfied(omsCoursePlan, isPast, type) {
                ...
                //一对多排课 需要传入schoolid
                $scope.OmsCoursePlanVoForCreate.schoolId = AuthenticationService.currentUser().school_id
                ...
                if ($scope.isSelect) {
                    SweetAlert.swal({
                        ...
                        if (confirm) {
                            ...
                            console.log($scope.OmsCoursePlanVoForCreate)
                            var promise = CoursePlanService.consume($scope.OmsCoursePlanVoForCreate);
                            promise.then(function (OmsCoursePlanVoForCreate) {
                                $scope.dataLoading = false;
                                if (OmsCoursePlanVoForCreate.status == 'FAILURE') {
                                    SweetAlert.swal('操作失败：'+OmsCoursePlanVoForCreate.error);
                                    return false
                                }
                                //成功后刷新页面
                                $scope.ListencallServerrecord($scope.myCoursePlanRecordTableState)
            ...
            function showEditCoursePlan(row, type) {
                ...
                //studentIdList  tempGroupId一对多排课所需要的参数
                $scope.OmsCoursePlanVoForEdit.studentIdList = row.studentIdList;
                $scope.OmsCoursePlanVoForEdit.tempGroupId = row.tempGroupId;
                ...
                $scope.select.timeSize = row.courseTime / 0.5;
            ...
            $scope.TIME_SIZE1 = [
                { id: 1, name: '0.5小时' }, { id: 7, name: '40分钟' }, { id: 2, name: '1小时' }, { id: 3, name: '1.5小时' }, { id: 4, name: '2小时' }, {
                    id: 5,
                    name: '2.5小时'
                }, { id: 6, name: '3小时' },
            ];
            ...
            $scope.search = function () {
                $scope.studentListForAddGroupTableState.pagination.start = 0
                $scope.studentListForAddGroupTableState.search.predicateObject.pageNum = 1
                $scope.myCrmCustomerStudentListForAddGroupFilter.pageNum = 1
                $scope.getMyCrmCustomerStudentListForAddGroup($scope.studentListForAddGroupTableState);
            }
