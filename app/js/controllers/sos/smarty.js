...
var app = angular.module('ywsApp');

app.controller("SmartyController", [
    "$scope", "$document", "smartySuggestor", "$window", "$timeout",'ProductService',
    function($scope, $document, smartySuggestor, $window, $timeout,ProductService) {
        ...
        */
        ...
        $scope.$watch("prefix", function(newValue, oldValue) {
            if (newValue != oldValue && $scope.selectionMade == false) {
                if ($scope.prefix == "" || angular.isUndefined($scope.prefix)) {
                    ...
                    var promise = ProductService.getCourseListByName($scope.prefix);
        ...
        $scope.inputfocused = function() {
            if ($scope.prefix && $scope.suggestions.length == 0 ) {
                $scope.isCourseListLoading = true;
                var promise = ProductService.getCourseListByName($scope.prefix);
                promise.then(function(data) {
                    $scope.suggestions = data.data;
        ...
        $scope.suggestionPicked = function() {
            if ($scope.selected != -1 && $scope.selected < $scope.suggestions.length) {
                $scope.prefix = $scope.suggestions[$scope.selected].name;
                $scope.$emit("chooseOne", $scope.suggestions[$scope.selected]);
            }
