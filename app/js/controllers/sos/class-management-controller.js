 ...
 * @version 1.0
 */
angular.module('ywsApp').controller('ClassManagementController', [
    ...
    'ClassStudentAttendenceService', 'ClassStudentRecordService', 'CustomerStudentService', '$mtModal', 'PromotionMTServer', 'ColEdit', 'DepartmentService', 'PositionService',
    function ($scope, $modal, $rootScope, SweetAlert, ClassManagementService, ProductService, OrderService,
            ...
            promise.then(function (response) {
                if (response.status == "FAILURE") {
                    ...
                    for (var i = 0; i < $scope.MyCrmCustomerStudentClassList.length; i++) {
                        // if($scope.MyCrmCustomerStudentClassList[i].courseSubjectType == 3){
                        //     $scope.MyCrmCustomerStudentClassList[i].classMultipleList = [];
                        //     var list = $scope.MyCrmCustomerStudentClassList[i].crmClassSubjectTeacher;
                        //     list.forEach(function(j,index){
                        //         j.detail.forEach(function(k){
                        //             $scope.MyCrmCustomerStudentClassList[i].classMultipleList.push(k);
                        //         })
                        //     })
                        // }
        ...
        $scope.doSelectCourse = function () {
            ...
            //判断是否是班类多科课程
            switch ($scope.selectedCourse.subjectType) {
                case 3:
            ...
            if ($scope.detailModal && $scope.detailModal.modalStatus === 'info') {
                ...
                promise.then(function (response) {
                    if (response.status == "FAILURE") {
                        ...
                        console.log($scope.studentClass.userName)
        ...
        $scope.createStudentClass = function () {
            var coursePlan = angular.copy($scope.LessonTimeListNewNewNew);
            coursePlan.forEach(function (i, index, array) {
                i.detail.forEach(function (j, jIndex, jArray) {
                    delete coursePlan[index].detail[jIndex].multipleCourseSelectTime;
                    delete coursePlan[index].detail[jIndex].subject;
                    delete coursePlan[index].detail[jIndex].teacher;
                    delete coursePlan[index].detail[jIndex].subjectName;
                    delete coursePlan[index].detail[jIndex].teacherName;
                    // delete coursePlan[index].detail[jIndex].timelongName;
                })
            })
            var teacherArray = [];
            $scope.multipleCourseTeacherLists.forEach(function (i) {
                var teacher = {};
                teacher.teacherId = i.teachTeacher.userId;
                teacher.subjectId = i.teachSubject.id;
                teacherArray.push(teacher);
            })
            if ($scope.selectedCourse) {
                if ($scope.selectedCourse.subjectType == 3) {
                    $scope.studentClass.classTimeJson = JSON.stringify(coursePlan);
                    $scope.studentClass.crmClassSubjectTeacher = teacherArray;
                }
            }
            ...
            if ($scope.isGenerateCoursePlan) {
                ...
                if ($scope.selectedCourse.subjectType == 3) {
                    ...
                    if ($scope.studentClass.crmClassSubjectTeacher.length == 0) {
            ...
            if ($scope.selectedCourse) {
                if ($scope.selectedCourse.subjectType == 3) {
                    for (var i = 0, len = classTimeList.length; i < len; i++) {
                        for (var j = 0; j < classTimeList[i].detail.length; j++) {
                            if (!et(classTimeList[i].detail[j].endTime, classTimeList[i].detail[j].startTime, classTimeList[i].detail)) {
                                return false
                            }
                        }
                    }
            ...
            $scope.studentClass.classCategory = 1;
        ...
        function showDetailModal(obj, selectTab) {
            ...
            $scope.studentClass.isEdit = false;
            $scope.show.planLists = JSON.parse($scope.studentClass.classTimeJson);
            $scope.paiKePlanLists = JSON.parse($scope.studentClass.classTimeJson);
            //初始化tab的url
        ...
        $scope.editCoursePlanSetting = function (studentClass, row) {

            if (studentClass.courseSubjectType == 3) {
                $scope.coursePlanEntrance = "editCoursePlan";
                $scope.editCoursePlanInfo = {
                    start: row.start,
                    startDate: row.startDate
                }

                $scope.week.forEach(function (i) {
                    if (i.startDate == row.startDate) {
                        $scope.onlyAllowModify = i.theDayOfWeekId;
                        $scope.activeDay = i.theDayOfWeekId - 1;
                    }
                })
                $scope.courseList = row.detail;
                $scope.LessonTimeList = [];
                $scope.courseList.forEach(function (i, index, array) {
                    i.subject = $scope.omsSubject;
                    i.multipleCourseSelectTime = {
                        subjectId: i.subjectId,
                        teacherId: i.teacherId,
                        time1: i.startTime.split(":")[0],
                        time2: i.startTime.split(":")[1],
                        timeSize: i.timelong,
                        timeEnd: i.endTime
                    }
                    $scope.LessonTimeList.push(i);
                })
                $scope.addMultipleCourseTitle = "修改上课计划";
                $scope.addMultipleCoursePlanTimeModal = $modal({
                    scope: $scope,
                    templateUrl: 'partials/sos/class/modal.addClassMultipleSubjectCoursePlanTime.html?' + new Date().getTime(),
                    show: true,
                    backdrop: "static"
                });
            } else {
                $scope.detailModal.hide();
                //展示编辑弹窗
                $scope.editModalTitle = "修改班级排课设置：" + studentClass.name;
                $scope.detailModal = $modal({
                    scope: $scope,
                    templateUrl: 'partials/sos/class/modal.edit.html?' + new Date().getTime(),
                    show: true,
                ...
                $scope.detailModal.modalStatus = 'edit';  //更改模态框状态
                //不是编辑班级全部信息，只是编辑排课设置
                studentClass.isEdit = false;
                studentClass.isEditCoursePlanSetting = true;
            }
        ...
        function update(crmStudentClass) {
            ...
            if (crmStudentClass.courseSubjectType != 3) {
                for (var i = 0, len = classTimeList.length; i < len; i++) {
                    if (!et(classTimeList[i].endTime, classTimeList[i].startTime, classTimeList[i])) {
                        return false
                    }
        ...
        $scope.saveNamed = function saveNamed() {
            var dataJson = {
                ...
                teacherList: $scope.coursemultiplemakearry
            }
        ...
        function submitPlan(classId) {
            if ($scope.selectedCourse.subjectType == 3) {
                var subjectLists = JSON.parse($scope.studentClass.classTimeJson);
                var list = [[], [], [], [], [], [], []];
                subjectLists.forEach(function (i, index, array) {
                    i.detail.forEach(function (j, jIndex, jArray) {
                        list[parseInt(j.day) - 1].push(j);
                    })
                })
                ...
                var plans = [];
                var multipleglag, multipleglaglist = 0;
                var weekNum = Math.ceil((pEndTimestamp - pStartTimestamp) / 1000 / 60 / 60 / 24 / 7);
                var weeksLength;
                if ($scope.studentClass.planCourseCount && $scope.studentClass.planCourseCount <= weekNum) {
                    weeksLength = $scope.studentClass.planCourseCount;
                } else {
                    weeksLength = weekNum;
                }
                var num = 0;
                    ...
                    for (var j = 0; j < lists.length; j++) {
                        ...
                        if (lists[j].length > 0) {
                            for (var k = 0; k < lists[j].length; k++) {
                                var dayOfWeek = lists[j][k].day;
                                var startTime = lists[j][k].startTime;
                                var endTime = lists[j][k].endTime;

                                var start = _getTimestampByWeekAndTimeNew2(dayOfWeek, startTime, pStartTimestamp + i * 7 * 24 * 3600 * 1000);
                                var end = _getTimestampByWeekAndTimeNew2(dayOfWeek, endTime, pStartTimestamp + i * 7 * 24 * 3600 * 1000);
                                if (j == 0) {
                                    start = parseInt(start) + 28800000;
                                }
                                // $scope.config = {
                                //     id: ''
                                //     , type: ''
                                ...
                                if (end <= pEndTimestamp && start >= pStartTimestamp) {
                                    var obj = {
                                        multipleglaglist: j,
                                        multipleglag: i
                                    }
                                    ...
                                    obj.start = start;
                                    obj.end = end;
                                    obj.subjectID = lists[j][k].subjectId;
                                    obj.teacherID = lists[j][k].teacherId;
                                    obj.teacherId = lists[j][k].teacherId;
        ...
        //上课老师
        //$scope.classMultipleInfo 用于添加班级，添加上课计划时，绑定科目和老师
        $scope.classMultipleInfo1 = {};
        $scope.classMultipleInfoList = [];
        //$scope.classMultipleInfo 用于添加班级，添加上课老师时，绑定科目和老师
        $scope.classMultipleInfo = {};
        //空对象，用于传到service，做数据封装
        $scope.multipleCourseTeachersFilter = {};
        //班类多科    科目和老师联动，选科目，请求对应的老师列表
        $scope.getMultipleCourseTeachers = function getMultipleCourseTeachers(subjectId, listIndex, entrance) {
            //$scope.subjectId 当前选中的科目id
            $scope.subjectId = subjectId;
            //通过科目id，遍历科目列表，拿到科目名称
            $scope.omsSubject.forEach(function (i) {
                if ($scope.subjectId == i.id) {
                    $scope.subjectName = i.name;
                }
            })
            //修改已经添加的上课计划时，会传入当前修改项的下标，如果只是添加的，就不传
            ...
            //设置tableState变量，用于传入service做数据封装
            var tableState = {
                pagination: {},
                search: {},
                sort: {}
            ...
            if (listIndex == void 0) {
                //如果入口为  添加课程-添加上课老师-添加 and 添加课程-添加上课计划-添加
                CustomerStudentCourseService.getCSShooleTeacherByFiltersNew(0, 0, tableState, $scope.multipleCourseTeachersFilter).then(function (result) {
                    //重置老师列表
                    $scope.multipleCourseTeachersList = result.data.studentTeachers.list;
                    //重置绑定的老师Id
                    $scope.classMultipleInfo.teacherId = null;
                    $scope.classMultipleInfo1.teacherId = null;
                    $scope.classMultipleInfo2.teacherId = null;
                    $scope.isLoading = false;
                });
            } else {
                //如果入口为  添加课程-添加上课计划-修改已添加的项
                CustomerStudentCourseService.getCSShooleTeacherByFiltersNew(0, 0, tableState, $scope.multipleCourseTeachersFilter).then(function (result) {
                    //重置当前修改项的老师列表
                    $scope.LessonTimeList[listIndex].teacher = result.data.studentTeachers.list;
                    //重置当前修改项的老师Id
                    if (entrance != 1) {
                        $scope.LessonTimeList[listIndex].multipleCourseSelectTime.teacherId = null;
                    }
        ...
        //改变科目对应的老师时，对应改变老师名字
        $scope.changeLessonTimeListTeacher = function (index, teacherId) {
            $scope.LessonTimeList[index].teacherId = teacherId;
            $scope.LessonTimeList[index].teacher.forEach(function (i) {
                if (teacherId == i.userId) {
                    $scope.LessonTimeList[index].teacherName = i.username;
                }
            })
        }
        //$scope.multipleCourseTeacherLists 用于 添加课程-添加上课老师 班级列表-修改-添加上课老师 老师列表
        $scope.multipleCourseTeacherLists = [];
        //添加上课老师  添加课程-添加上课老师 班级列表-修改-添加上课老师
        $scope.addMultipleCourseTeachers = function () {

            if ($scope.subjectId == void 0) {
                //没有选择科目
                warningAlert("请选择科目");
                return;
            } else if ($scope.classMultipleInfo1.teacherId == void 0) {
                //没选择上课老师
                warningAlert("请选择老师");
                return;
            }
            //判断是否已经添加了上课老师
            for (var i = 0; i < $scope.multipleCourseTeacherLists.length; i++) {
                if ($scope.multipleCourseTeacherLists[i].teachTeacher.userId == $scope.classMultipleInfo1.teacherId && $scope.multipleCourseTeacherLists[i].teachSubject.id == $scope.classMultipleInfo1.subjectId) {
                    warningAlert("您已添加该科目上课老师");
                    return;
                }
            }
            //teacherInfo 老师信息：包含科目和上课老师
            var teacherInfo = {};
            //遍历当前科目对应的老师列表，将当前选中的老师对象添加到teacherInfo
            $scope.multipleCourseTeachersList.forEach(function (i) {
                if ($scope.classMultipleInfo1.teacherId == i.userId) {
                    teacherInfo.teachTeacher = i;
                }
            })
            //遍历科目列表，将当前选中的科目兑现添加到teacherInfo
            $scope.omsSubject.forEach(function (i) {
                if ($scope.subjectId == i.id) {
                    teacherInfo.teachSubject = i;
                }
            })
            //向上课老师列表中添加当前选中的上课老师
            $scope.multipleCourseTeacherLists.push(teacherInfo);
            //添加后重置老师列表
            $scope.multipleCourseTeachersList = [];
            //添加后重置当前
            $scope.classMultipleInfo1 = {};
        }
        //删除当前选中的上课老师列表项 添加课程-上课老师
        $scope.deleteMultipleSubjectTeachers = function (index) {
            $scope.multipleCourseTeacherLists.splice(index, 1)
        }
        //上课计划
        //定义星期列表
        $scope.week = [
        ...
        //选择星期
        //$scope.activeDay 初始化选中的星期 也用于页面的class控制
        $scope.activeDay = null;
        //点击每个星期，设置当前的星期状态，index、startDate为必选项
        $scope.chooseTheDayOfWeek = function (index, startDate) {
            //设置当前选中的星期
            $scope.activeDay = index;
            //设置当前选中的星期名称
            $scope.startDate = startDate;
            //置空用于添加时绑定科目和老师的变量 $scope.classMultipleInfo
            $scope.classMultipleInfo = {};
            //$scope.multipleCourseSelectTime 用于添加时绑定上课时间，课时，下课时间
            $scope.multipleCourseSelectTime = {};
            //重置老师列表
            $scope.multipleCourseTeachersList = [];
        }
        //初始化用于当定上课时间、课时、下课时间的变量
        $scope.multipleCourseSelectTime = {};
        ...
        //通过上课时间、课时来计算下课时间，listIndex为可选项
        $scope.getMultipleCourseEndTime = function (listIndex) {
            ...
            if (listIndex == void 0) {
                if (!$scope.classMultipleInfo.teacherId || (!$scope.classMultipleInfo.subjectId && $scope.classMultipleInfo.subjectId != 0) || $scope.classMultipleInfo.subjectId == 0) {
            ...
            if ((dayOfWeek + 1) && (changeWhichOne.time1 || changeWhichOne.time1 == 0) && changeWhichOne.timeSize || listIndex != void 0) {
                ...
                //校验上下课时间是否隔天 checkStatus为返回的状态 false or true
            ...
            if (listIndex != void 0) {
                $scope.LessonTimeList[listIndex].startTime = changeWhichOne.time;
                $scope.LessonTimeList[listIndex].endTime = changeWhichOne.timeEnd;
                $scope.LessonTimeList[listIndex].timelong = changeWhichOne.timeSize;
            }

        }
        //$scope.LessonTimeList 用于添加上课计划页面的列表展示 为平铺结构 含本次添加的所有数据
        $scope.LessonTimeList = [];
        //添加上课计划弹框，点添加按钮执行
        $scope.addLessonTime = function () {
            ...
            //lessonTimeInfo 用于封装单个上课计划
            var lessonTimeInfo = {};
            //lessonTimeInfo.multipleCourseSelectTime 用于设置页面绑定的上课计划信息 含教师id 科目id 起始时间的小时及分钟 课时 下课时间
            ...
            //通过课时id：timeSize 从$scope.TIME_SIZE1中拿到课时名称
            ...
            //校验本次添加的上课计划
            for (var i = 0; i < $scope.LessonTimeList.length; i++) {
                if ($scope.LessonTimeList[i].day == $scope.activeDay + 1) {
                    var startTime2 = new Date("1970/1/2 " + $scope.LessonTimeList[i].startTime);
                    var endTime2 = new Date("1970/1/2 " + $scope.LessonTimeList[i].endTime);
                    if (startTime1 >= startTime2 && startTime1 <= endTime2 || endTime1 >= startTime2 && endTime1 <= endTime2) {
                        warningAlert("时间重复");
                        return;
                    }
         ...
         * 确认添加课程
         * */
        //$scope.LessonTimeListNewNewNew 用于新增班级-上课计划列表的展示
        $scope.LessonTimeListNewNewNew = [];
        // $scope.LessonTimeListNew = [];
        //添加上课计划页面  点确定按钮触发
        $scope.addMultipleCoursePlanTime = function () {

            if ($scope.coursePlanEntrance == 'editCoursePlan') {
                //入口为  班级列表-查看-修改-修改设置
                //$scope.show.planLists 此时的值为班级列表-查看带过来的计划列表，结构与$scope.LessonTimeListNewNewNew类似
                $scope.LessonTimeListNewNewNew = angular.copy($scope.show.planLists);
                var afterModifyLesson = {};
                afterModifyLesson.start = $scope.editCoursePlanInfo.start;
                afterModifyLesson.startDate = $scope.editCoursePlanInfo.startDate;
                afterModifyLesson.detail = [];
                $scope.LessonTimeList.forEach(function (i, index, array) {
                    var j = {};
                    j.day = i.day;
                    j.endTime = i.endTime;
                    j.start = i.start;
                    ...
                    j.subjectId = i.subjectId;
                    j.subjectName = i.subjectName;
                    j.teacherId = i.teacherId;
                    j.teacherName = i.teacherName;
                    j.timelong = i.timelong;
                    j.timelongName = i.timelongName;
                    afterModifyLesson.detail.push(j);
                })

                $scope.LessonTimeListNewNewNew.splice($scope.editCoursePlanInfo.start, 1, afterModifyLesson);
                if (afterModifyLesson.detail.length == 0) {
                    $scope.LessonTimeListNewNewNew.splice(afterModifyLesson.start, 1);
                }
                //校验时间是否冲突
                var checkLessonTime = $scope.LessonTimeListNewNewNew;
                //将所有时间提取出来
                var checkLists = [[], [], [], [], [], [], []];
                ...
                $scope.show.planLists = angular.copy($scope.LessonTimeListNewNewNew);
                $scope.paiKePlanLists = $scope.show.planLists;


                var classTimeJson = JSON.stringify($scope.show.planLists)
                $scope.studentClass.classTimeJson = classTimeJson;
            ...
            //lists 将本次添加的上课计划按星期分开
            var lists = [[], [], [], [], [], [], []];
            $scope.LessonTimeList.forEach(function (i, index, array) {
                lists[parseInt(i.day) - 1].push(i);
            })
            //将按星期分开后的上课计划lists重组成纵向结构
            $scope.LessonTimeListNewNew = [];
            //startIndex 为纵向结构的上课计划添加下标start
            ...
            // })
            // //$scope.LessonTimeListNewNew 用于将本次添加的所有课程组装成后端需要的固定格式
            // $scope.LessonTimeListNewNew = [];
            // var startIndex = $scope.LessonTimeListNewNewNew.length;
            // lists.forEach(function (i, index, array) {
            //     if (i.length > 0) {
            ...
            //$scope.LessonTimeListNewNewNew 用于添加课程页面的上课计划列表展示，也用于传入后端的数据
            $scope.LessonTimeListNewNewNew = $scope.LessonTimeListNewNewNew.concat($scope.LessonTimeListNewNew);
            //$scope.multiplelistdetail 用于排课添加页面，上课计划展示
            if ($scope.multiplelistdetail) {
                $scope.multiplelistdetail = $scope.multiplelistdetail.concat($scope.LessonTimeListNewNew);
            }
            $scope.addMultipleCoursePlanTimeModal.hide();
            //重置变量
            console.log($scope.stucentClass)
            clearAllMultipleCourseData();
        }
        //添加课程页面删除上课计划列表
        $scope.deleteMultipleCourseList = function (index) {
            $scope.LessonTimeListNewNewNew.splice(index, 1);
            // $scope.selectedCourse = null;
        }
        //重置数据
        ...
        $scope.clearAllMultipleCourseData1 = clearAllMultipleCourseData1;
        function clearAllMultipleCourseData1() {
            $scope.multipleCourseTeacherLists = [];
        }

        // $scope.$watch("addMultipleCoursePlanTimeModal.$isShown",function(old,newval){
        //     if(old != newval){
        //         clearAllMultipleCourseData();
        //     }
        // })
        ...
        // //查看上课时间
        $scope.viewCoursePlanTimeTimeModal = function (row) {
            $scope.multipleCoursePlanTimeList = [row];
            $scope.multipleCoursePlanTimeList.forEach(function (i, index) {
                i.detail.forEach(function (j, jIndex) {
                    $scope.TIME_SIZE1.forEach(function (k, kIndex) {
                        if (k.id == j.timelong) {
                            $scope.multipleCoursePlanTimeList[index].detail[jIndex].timelongName = k.name;
                        }
                    })
                })
            })
            $scope.viewCoursePlanTimeTiele = "查看";
            $scope.viewCoursePlanTimeModal = $modal({
                scope: $scope,
                templateUrl: 'partials/sos/class/modal.viewCoursePlanTime.html',
                show: true,
                backdrop: "static"
            });
        }
        ...
        //班级列表查看上课时间
        $scope.viewCourseTimeList = function (row) {
            var classTimeJson = JSON.parse(row.classTimeJson);
            if (!classTimeJson) {
                return;
            }
            classTimeJson.forEach(function (i, index, array) {
                i.detail.forEach(function (j, jIndex, jArray) {
                    $scope.TIME_SIZE1.forEach(function (k, kIndex, kArray) {
                        if (k.id == j.timelong) {
                            classTimeJson[index].detail[jIndex].timelongName = k.name;
                        }
                    })
                })
            })
            $scope.multipleCoursePlanTimeList = classTimeJson;
            $scope.viewCoursePlanTimeTiele = "查看";
            $scope.viewCoursePlanTimeModal = $modal({
                scope: $scope,
                templateUrl: 'partials/sos/class/modal.viewCoursePlanTime.html',
        ...
        //班级详情，删除上课老师
        $scope.classMultipleInfo2 = {};
        $scope.deleteClassDetailTeacher = function (index) {
            $scope.studentClass.crmClassSubjectTeacher.splice(index, 1);
        }
        $scope.addClassDetailTeacher = function () {
            for (var i = 0; i < $scope.studentClass.crmClassSubjectTeacher.length; i++) {
                if ($scope.studentClass.crmClassSubjectTeacher[i].subjectId == $scope.classMultipleInfo2.subjectId && $scope.studentClass.crmClassSubjectTeacher[i].teacherId == $scope.classMultipleInfo2.teacherId) {
                    warningAlert("您已添加该上课老师");
                    return false;
                }
            }
            ...
            if ($scope.subjectId == void 0) {
                warningAlert("请选择科目");
                return;
            } else if ($scope.classMultipleInfo2.teacherId == void 0) {
                warningAlert("请选择老师");
                return;
            }
            var teacher = {};
            teacher.createdAt = null;
            teacher.createdBy = null;
            teacher.updateAt = null;
            teacher.updateBy = null;
            teacher.subjectName = $scope.subjectName;
            teacher.subjectId = $scope.subjectId;
            teacher.teacherId = $scope.classMultipleInfo2.teacherId;
            $scope.multipleCourseTeachersList.forEach(function (i) {
                if (i.userId == teacher.teacherId) {
                    teacher.teacherName = i.username;
                    return;
                }
            })

            $scope.studentClass.crmClassSubjectTeacher.push(teacher);
            $scope.multipleCourseTeachersList = [];
            $scope.classMultipleInfo2 = {};
        }
        //author:zhanghonjie 班类多科部分
        //        班类多科排课modald打开

        $scope.multipleAddCoursePlanInfo = function (row, type) {
            $scope.detail = angular.copy(row);

            $scope.rowlink = row;
            $scope.multiplelistdetail = JSON.parse(row.classTimeJson)
            $scope.show.planLists = $scope.multiplelistdetail;
            ...
            CommonService.getSubjectIdSelect().then(function (result) {
                ...
                console.log($scope.omsSubject)
                $scope.recordCoursePlanModal = $modal({
                    scope: $scope,
                    templateUrl: 'partials/sos/customer/multipleAddCoursePlanInfo.html',
                    show: true,
                    backdrop: "static"
                });
            });
        ...
        $scope.searchSpecify1 = function (changeSchoolName, schoolNature) {
            var params = {
            ...
            AuthenticationService.getSchool(params).then(function (response) {
                if (response.status == "SUCCESS") {
                    ...
                    if (response.data.schoolList.length != 0) {
                        $scope.departments = response.data.schoolList
                    }
                }
            })
        }
        // var datasFilters = []
        function searchSpecifyLocal(name,list,filters) {
            var datas = list || getLocalDatas()
            var datasFilters = filters||[]
            for (var i = 0,len=datas.length ; i<len;i++) {
                if (datas[i].name.indexOf(name)>-1) {
                    datasFilters.push(datas[i])
                } else if (datas[i].children){
                    searchSpecifyLocal(name,datas[i].children,datasFilters)
                }
            }
           $scope.departments = datasFilters
        }
        $scope.searchSpecifyLocal = searchSpecifyLocal
        ...
        function getLocalDatas() {
            var datas = sessionStorage.getItem('__departments__')
            if (datas) {
        ...
        function getAllDepartments() {
            var datas = getLocalDatas()
            if (datas) {
                $scope.departments = datas
                return false
            }
            var promise = DepartmentService.listV2(AuthenticationService.currentUser().organization.id);
            promise.then(function (response) {
                if (response.status == "FAILURE") {
                    SweetAlert.swal(response.data, "请重试", "error");
                }
                    ...
                    sessionStorage.setItem('__departments__',JSON.stringify(response.data))
        ...
        function selectNode(node) {
            if (node && !node.children && node.isSchool == 1) {
                // $rootScope.__organization__id = $rootScope.__organization__id||AuthenticationService.currentUser().organization.id
                $rootScope.curDepartId = node.id;
                $rootScope.toJumpSchoolName = node.name;
