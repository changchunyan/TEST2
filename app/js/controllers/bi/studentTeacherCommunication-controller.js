...
/**
 * The studentTeacherCommunication controller.
 * @version 1.0
 */
angular.module('ywsApp').controller('BiStudentTeacherCommunicationController', [
	'$scope', '$modal', '$filter', '$rootScope', 'SweetAlert', 
	'BiStudentTeacherCommunicationService', 'DepartmentService',
	...
	function($scope, $modal, $filter, $rootScope, SweetAlert, 
		...
		function changeCurrentTab(obj){
			if(obj.title==='教师日评'){
				$scope.currentTab=1;
			}else if(obj.title==='教师月评'){
				$scope.currentTab=2;
			}else if(obj.title==='评价汇总'){
				$scope.currentTab=3;
			}
		}
		/**
		...
		function getStatistics(tableState) {
            ...
            if($scope.currentTab===1){
            	$scope.searchModel.commentType = 'D1';
            }else if($scope.currentTab===2){
            	$scope.searchModel.commentType = 'M1';
            }else if($scope.currentTab===3){
            	$scope.searchModel.commentType = 'D1';
            }
            BiStudentTeacherCommunicationService.getPageList($scope.searchModel)
		...
		function getStatisticsAll() {
			...
			if ($scope.statisticsTableState.pagination.numberOfPages > 3000) {
			...
			BiStudentTeacherCommunicationService.getAllList($scope.searchModel)
				.then(function (result) {
			...
			if($scope.currentTab===1){
				statisticsExportTableStyle = {
						...
						columns: [
						          ...
						          {columnid:'teacherName',title: '教师姓名'},
						          {columnid:'lessonStartDate',title: '上课日期'},
						          {columnid:'lessonStartTime',title: '上课时间'},
						          {columnid:'studentName', title: '学生姓名'},
						          {columnid:'gradeName',title: '年级'},
						          {columnid:'subjectName',title: '科目'},
						          {columnid:'lessonPreparationGrade',title: '综合评分'},
						          {columnid:'recommendation',title: '建议'},
				...
				statisticsExportTableStyle = {
						...
						columns: [
						          ...
						          {columnid:'gradeName',title: '年级'},
						          {columnid:'subjectName',title: '科目'},
						          {columnid:'knowledgeMasterGrade',title: '提分'},
						          {columnid:'summaryPlanGrade',title: '满意'},
		...
		function getSummary(tableState) {
            ...
            BiStudentTeacherCommunicationService.getSummaryPageList($scope.searchSummaryModel)
        	.then(function (result) {
			...
			BiStudentTeacherCommunicationService.getComments($scope.searchSummaryModel)
	        	.then(function (result) {
	                $scope.presentTotal = {};
		...
		function getSummaryAll() {
            ...
            BiStudentTeacherCommunicationService.getSummaryAllList($scope.searchSummaryModel)
	        	.then(function (result){
			...
			if($scope.viewDivision){
				summaryExportTableStyle = {
					...
					columns: [{columnid:'schoolDivision',title: '事业部',width: '100px'},
					          ...
					          {columnid:'satisfiedGrade',title: '月满意度'},
					          {columnid:'improvementGrade',title: '月提分率'},
					          {columnid:'feedBackGrade',title: '月反馈率'}
					         ],
			        row: {
			        	style: function(sheet, row, rowidx){
			        		return 'background:'+(rowidx%2?'#E1FFFF':'#F0E68C');
				...
				summaryExportTableStyle = {
				        ...
				        column: {style:'font-size:14px; text-align:left;'},
				        columns: [{columnid:'teacherName',title: '教师'},
				                  {columnid:'averageGrade',title: '月均分'},
						          {columnid:'satisfiedGrade',title: '月满意度'},
						          {columnid:'improvementGrade',title: '月提分率'},
						          {columnid:'feedBackGrade',title: '月反馈率'}
		...
		function getData(obj){
			BiStudentTeacherCommunicationService.getSummaryPageList(obj)
	        	.then(function (result) {
	                $scope.summaryModels = result.data.list;
	                angular.forEach($scope.summaryModels, function(data, index){
	                	if(data.averageGrade != null){
			...
			BiStudentTeacherCommunicationService.getComments(obj)
	        	.then(function (result) {
	                $scope.presentTotal = {};
	                $scope.presentTotal.averageGrade = result.data.averageGrade.toFixed(2);
	                $scope.presentTotal.satisfiedGrade = (result.data.satisfiedGrade*100).toFixed(2) + '%';
