...
/**
 * The biTeacherConsumeAnalysis controller.
 * @version 1.0
 */
angular.module('ywsApp').controller('BiTeacherConsumeAnalysisController', [
	'$scope', '$modal', '$filter', '$rootScope', 'SweetAlert', 'BiTeacherConsumeAnalysisService', 'DepartmentService',
	'AuthenticationService', 'localStorageService',
	function($scope, $modal, $filter, $rootScope, SweetAlert, BiTeacherConsumeAnalysisService, DepartmentService,
			AuthenticationService, localStorageService) {
        ...
        /**
    		 ...
    		 * 根据校区id获取校区信息
    		 */
    		function querySchoolById(schoolId, callback) {
    		 ...
    		 * 获取操作用户信息
    		 */
    		function getLoginUserInfo(callback) {
    			...
    			querySchoolById(department.id, callback);
    		}
    		/**
    		 * 获取事业部名称
    		 */
        ...
        $scope.getTeacherClassHour = getTeacherClassHour;
        ...
        function summaryByDate(consumeList) {
          ...
          for (var i = 0; i < consumeList.length; i++) {
            ...
            if (lastDate != consume.consumeDate) {
              ...
              lastSummary.numberOfTeachers = 1;
            } else {
              lastSummary.totalConsumeHour += consume.totalConsumeHour;
              lastSummary.numberOfTeachers += 1;
            }
          }
          return summary;
...
//          analyzeAllStudents(summary);
        ...
        function summaryByDateAndSubject(consumeList) {
          ...
          for (var i = 0; i < consumeList.length; i++) {
            ...
            var summaries = summaryBySubject[consume.subjectName];
            if (summaries == null) {
              summaries = [];
              for (var j = 0; j < dates.length; j++) {
                summaries.push({consumeDate: dates[j], totalConsumeHour: 0, numberOfTeachers: 0});
              }
              summaryBySubject[consume.subjectName] = summaries;
            }
            for (var j = 0; j < summaries.length; j++) {
              if (summaries[j].consumeDate == consume.consumeDate) {
                summaries[j].totalConsumeHour = summaries[j].totalConsumeHour + consume.totalConsumeHour;
                summaries[j].numberOfTeachers = summaries[j].numberOfTeachers + 1;
                break;
              }
            }
          }
          ...
          for (var i = 0; i < subjects.length; i++) {
            ...
            for (var j = 0; j < summaries.length; j++) {
              summaries[j].averageClassHourCountPerTeacher = summaries[j].totalConsumeHour / summaries[j].numberOfTeachers;
            }
          }
        ...
        function showSubjectDrillDown(response) {
          ...
          for (var i = 0; i < subjects.length; i++) {
            ...
            for (var j = 0; j < consumes.length; j++) {
              lineData.push(consumes[j].totalConsumeHour);
              if (consumes[j].numberOfTeachers == 0) {
                lineDataAverage.push(0);
              } else {
                lineDataAverage.push(consumes[j].totalConsumeHour / consumes[j].numberOfTeachers);
              }
              lineStudentsData.push(consumes[j].numberOfTeachers);
            }
...
//          analyzeSubjectStudents(summary, dates);
        ...
        function getTeacherClassHour() {
          ...
          $scope.analysisStudent = [];
          $scope.suggestionsStudent = [];
          BiTeacherConsumeAnalysisService.getTeacherClassHour($scope.startDate, $scope.endDate, $scope.viewType, $scope.searchModel.schoolId || $scope.searchModel.divisionId || localStorageService.get('department').id)
          .then(function(response) {
            $scope.orderedTeacherConsumeList = [];
            var consumeByTeacher = {};
            for (var i = 0; i < response.data.length; i++) {
              var consume = consumeByTeacher[response.data[i].teacherName];
              if (response.data[i].statType == $scope.viewType) {
                if (!consume) {
                  consume = {
                    teacherName: response.data[i].teacherName,
                    subjectName: response.data[i].subjectName,
                    totalConsumeHour: response.data[i].totalConsumeHour,
                  };
                  consumeByTeacher[response.data[i].teacherName] = consume;
                } else {
                  consume.totalConsumeHour += response.data[i].totalConsumeHour;
                }
              }
            }
            var consumeList = [];
            var teachers = Object.keys(consumeByTeacher);
            for (var i = 0; i < teachers.length; i++) {
              consumeList.push(consumeByTeacher[teachers[i]]);
            }
            consumeList.sort(function(a, b) {
              return b.totalConsumeHour - a.totalConsumeHour;
            });
            $scope.orderedTeacherConsumeList = consumeList;
        ...
        $scope.options = {
          ...
          tooltips: {
    		...
    		setTimeScope(0);
    		getAllProvinces();
    		getLoginUserInfo(getTeacherClassHour);
	}
]);
